...
public partial class ResourceCompressionTool
{
    ...
    private void DrawTextureListSection()
    {
        EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.MaxWidth(position.width));
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    EditorGUILayout.LabelField($"({selectedTextures.Count}个)", new GUIStyle(EditorStyles.miniLabel) { normal = { textColor = Color.gray } }, GUILayout.Width(40));

                    long totalMemory = 0;
                    foreach (var texture in selectedTextures) totalMemory += CalculateTextureMemoryUsage(texture, null);
    ...
    private void DrawTextureList()
    {
        ...
        for (int i = 0; i < selectedTextures.Count; i++)
        {
            ...
            string textureKey = texturePath + i;

            if (!textureItemFoldouts.ContainsKey(textureKey)) textureItemFoldouts[textureKey] = false;
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                if (textureItemFoldouts[textureKey])
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        EditorGUILayout.LabelField(texture.mipmapCount > 1 ? $"是({texture.mipmapCount}级)" : "否", GUILayout.Width(60));
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button("定位", GUILayout.Width(40), GUILayout.Height(18))) EditorGUIUtility.PingObject(texture);
                        if (GUILayout.Button("×", new GUIStyle(GUI.skin.button) { fontSize = 12, fontStyle = FontStyle.Bold }, GUILayout.Width(24), GUILayout.Height(18)))
                        {
                            ...
                            GUIUtility.ExitGUI(); // Exit GUI to avoid layout errors after removing an item
                        }
                    }
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.BeginHorizontal();
    ...
    private void DrawAudioSettings()
    {
        EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.MaxWidth(position.width));
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                GUILayout.FlexibleSpace();
                if (selectedAudioClips.Count > 0 && GUILayout.Button("应用设置", GUILayout.Height(20), GUILayout.Width(80)))
                {
                    ApplyAudioSettings();
                }
            }
    ...
    private void DrawAudioListSection()
    {
        EditorGUILayout.BeginVertical(EditorStyles.helpBox, GUILayout.MaxWidth(position.width));
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    EditorGUILayout.LabelField($"({selectedAudioClips.Count}个)", new GUIStyle(EditorStyles.miniLabel) { normal = { textColor = Color.gray } }, GUILayout.Width(40));

                    long totalMemory = 0;
                    foreach (var audio in selectedAudioClips) totalMemory += CalculateAudioMemoryUsage(audio);
                    float totalMemoryMB = totalMemory / (1024f * 1024f);
                    GUIStyle memoryStyle = new GUIStyle(EditorStyles.miniBoldLabel) { normal = { textColor = totalMemoryMB > 100 ? Color.red : totalMemoryMB > 50 ? Color.yellow : Color.green } };
                    EditorGUILayout.LabelField($"{totalMemoryMB:0.0}MB", memoryStyle, GUILayout.Width(60));
    ...
    {
        EditorGUILayout.BeginVertical();
        for (int i = 0; i < selectedAudioClips.Count; i++)
        {
            AudioClip audio = selectedAudioClips[i];
            if (audio == null) continue;

            string audioPath = AssetDatabase.GetAssetPath(audio);
            string audioKey = audioPath + i;

            if (!audioItemFoldouts.ContainsKey(audioKey)) audioItemFoldouts[audioKey] = false;
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                long memoryUsage = CalculateAudioMemoryUsage(audio);
                float memoryMB = memoryUsage / (1024f * 1024f);
                ...
                if (GUI.Button(buttonRect, buttonContent, buttonStyle))
                {
                    audioItemFoldouts[audioKey] = !audioItemFoldouts[audioKey];
                }

                Rect memoryRect = new Rect(buttonRect.xMax - 55, buttonRect.y + 6, 50, 16);
                GUIStyle memoryStyle = new GUIStyle(EditorStyles.miniLabel) { normal = { textColor = memoryMB > 20 ? Color.red : memoryMB > 10 ? Color.yellow : Color.green }, alignment = TextAnchor.MiddleRight, fontStyle = FontStyle.Bold };
                GUI.Label(memoryRect, $"{memoryMB:0.0}MB", memoryStyle);

                if (audioItemFoldouts[audioKey])
                {
                    EditorGUILayout.Space(2);
                    EditorGUILayout.BeginHorizontal();
                    {
                        EditorGUILayout.LabelField("时长:", GUILayout.Width(35));
                        EditorGUILayout.LabelField($"{audio.length:0.0}s", GUILayout.Width(50));
                        EditorGUILayout.LabelField("频率:", GUILayout.Width(35));
                        EditorGUILayout.LabelField($"{audio.frequency}Hz", GUILayout.Width(70));
                        EditorGUILayout.LabelField("声道:", GUILayout.Width(35));
                        EditorGUILayout.LabelField($"{audio.channels}", GUILayout.Width(30));
                        GUILayout.FlexibleSpace();
                        if (GUILayout.Button("定位", GUILayout.Width(40), GUILayout.Height(18))) EditorGUIUtility.PingObject(audio);
                        if (GUILayout.Button("×", new GUIStyle(GUI.skin.button) { fontSize = 12, fontStyle = FontStyle.Bold }, GUILayout.Width(24), GUILayout.Height(18)))
                        {
                            selectedAudioClips.RemoveAt(i);
                            audioItemFoldouts.Remove(audioKey);
                            GUIUtility.ExitGUI();
                        }
                    }
                    EditorGUILayout.EndHorizontal();
        ...
        EditorGUILayout.BeginVertical(EditorStyles.helpBox);
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                switch (currentPage)
                {
                    case CompressionPage.Texture:
                        buttonText = "开始图片压缩";
                        isEnabled = selectedTextures.Count > 0;
                        break;
                    case CompressionPage.Audio:
                        buttonText = "开始音频压缩";
                        isEnabled = selectedAudioClips.Count > 0;
                        break;
                    case CompressionPage.Model:
                        buttonText = "开始模型压缩";
                        isEnabled = false;
