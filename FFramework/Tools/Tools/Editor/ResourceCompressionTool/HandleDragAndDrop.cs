...
public partial class ResourceCompressionTool
{
    ...
    private void ProcessDroppedAudioObjects(UnityEngine.Object[] droppedObjects)
    {
        ...
        foreach (var obj in droppedObjects)
        {
            ...
            {
                if (!selectedAudioClips.Contains(audio))
                {
                    selectedAudioClips.Add(audio);
                    addedCount++;
                }
            }
            else if (AssetDatabase.IsValidFolder(AssetDatabase.GetAssetPath(obj)))
    ...
    private int AddAudioFromFolderPath(string folderPath)
    {
        ...
        foreach (string guid in guids)
        {
            ...
            if (audio != null && !selectedAudioClips.Contains(audio))
            {
                selectedAudioClips.Add(audio);
                addedCount++;
            }
    ...
    private void ApplyTextureSettings()
    {
        ...
        if (!EditorUtility.DisplayDialog("确认应用设置", $"确定要将设置应用到 {selectedTextures.Count} 张图片吗？", "确定", "取消"))
        {
            return;
        }
        ...
        try
        {
            ...
            for (int i = 0; i < selectedTextures.Count; i++)
            {
                ...
                EditorUtility.DisplayProgressBar("应用设置", $"正在处理 {Path.GetFileName(path)}...", (float)(i + 1) / selectedTextures.Count);
            }
        }
        finally
        {
    ...
    private void ApplyAudioSettings()
    {
        ...
        if (!EditorUtility.DisplayDialog("确认应用设置", $"确定要将设置应用到 {selectedAudioClips.Count} 个音频吗？", "确定", "取消"))
        {
            return;
        }
            ...
            for (int i = 0; i < selectedAudioClips.Count; i++)
            {
                ...
                if (AssetImporter.GetAtPath(path) is AudioImporter importer)
                {
                    ...
                    if (audioSettings.overridePlatformSettings)
                    {
                        ...
                        var platformSettings = sampleSettings; // 使用相同的设置
                        importer.SetOverrideSampleSettings(platformName, platformSettings);
                    }
                    else
                    {
                        // 清除平台覆盖设置 - 设置为默认，不覆盖
                ...
                EditorUtility.DisplayProgressBar("应用设置", $"正在处理 {Path.GetFileName(path)}...", (float)(i + 1) / selectedAudioClips.Count);
            }
        }
        finally
        {
    ...
    {
        if (selectedAudioClips.Count == 0) return;
        string path = AssetDatabase.GetAssetPath(selectedAudioClips[0]);
        if (AssetImporter.GetAtPath(path) is AudioImporter importer)
        {
            var sampleSettings = importer.defaultSampleSettings;
        ...
        try
        {
            ...
            for (int i = 0; i < selectedTextures.Count; i++)
            {
                ...
                if (AssetImporter.GetAtPath(path) is TextureImporter importer)
                {
                    ...
                    if (textureSettings.forceCompression && importer.textureCompression == TextureImporterCompression.Uncompressed)
                    {
                ...
                EditorUtility.DisplayProgressBar("纹理压缩", $"正在处理 {Path.GetFileName(path)}", (float)(i + 1) / selectedTextures.Count);
            }
        }
        finally
        {
    ...
    {
        // 复用ApplyAudioSettings的逻辑，因为它本质上就是压缩过程
        ApplyAudioSettings();
    }
