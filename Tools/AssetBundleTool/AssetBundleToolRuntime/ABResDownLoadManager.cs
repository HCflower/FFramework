...
namespace FFramework
{
    ...
    public class ABResDownLoadManager : SingletonMono<ABResDownLoadManager>
    {
        ...
        /// </summary>
        public void CheckResUpdate(Action<bool> resUpdateSuccess, Action<string> resUpdateInfo)
        {
            //加载远端资源对比文件
            DownLoadAssetBundlesCompareFile((downloadOver) =>
            {
                if (downloadOver)
                {
                    ...
                    GetLocalResCompareFile((isOver) =>
                    {
                        if (isOver)
                        {
                            ...
                            //下载待更新列表downloadAssetBundleList中的AssetBundles
                            DownloadAssetBundlesFile(
                            (downloadOver) =>
                            {
                                if (downloadOver)
                                {
                                    //@下载完成,更新本地资源对比文件(将远端读取出来的对比文件覆盖本地)
                                    File.WriteAllText(Application.persistentDataPath + "/AssetBundleCompareInfo.txt", remoteInfo);
                                }
                                resUpdateSuccess(downloadOver);
                                Debug.Log(downloadOver == true ? "<color=green>所有资源下载完成</color>" : "<color=red>资源下载失败!请检查网络连接.</color>");
                            },
        ...
        //下载资源对比文件
        private async void DownLoadAssetBundlesCompareFile(Action<bool> downloadOver)
        {
            bool isOver = false;
            ...
            while (!isOver && reDownloadCount > 0)
            {
                ...
                {
                    //下载资源对比文件 - 保存到可读写文件夹中
                    isOver = DownloadFile(ResServerPath, "AssetBundleCompareInfo.txt", localPath + "/AssetBundleCompareInfo_TMP.txt");
                });
                reDownloadCount -= 1;
            }
            downloadOver?.Invoke(isOver);
        }
        ...
        //获取本地资源对比文件
        private void GetLocalResCompareFile(Action<bool> isOver)
        {
            //先判断persistentDataPath文件夹中是否有资源对比文件
            if (File.Exists(Application.persistentDataPath + "/AssetBundleCompareInfo.txt"))
            {
                StartCoroutine(GetLocalResCompareFile("file:///" + Application.persistentDataPath + "/AssetBundleCompareInfo.txt", isOver));
            }
...
#endif
                StartCoroutine(GetLocalResCompareFile(path + "/AssetBundleCompareInfo.txt", isOver));
            }
            //没有资源对比文件
            else
                isOver(true);
        ...
        //获取本地资源对比文件
        private IEnumerator GetLocalResCompareFile(string filePath, Action<bool> isOver)
        {
            //加载本地资源对比文件
        ...
        //下载AssetBundles文件
        private async void DownloadAssetBundlesFile(Action<bool> downloadOver, Action<float, string, string> downloadProgress)
        {
            //下载资源文件保存位置
            string localPath = Application.persistentDataPath;
            bool isOver = false;
            ...
            while (downloadResList.Count > 0 && reDownloadCount > 0)
            {
                for (int i = 0; i < downloadResList.Count; i++)
                {
                    ...
                    await Task.Run(() =>
                    {
                        isOver = DownloadFile(ResServerPath, downloadResList[i], localPath + "/" + downloadResList[i]);
                    });
                    if (isOver)
                    {
                        // 更新已下载大小
